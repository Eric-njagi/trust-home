...
import { authApi, chatApi, clientApi, workerApi } from '../services/apiClient.js';
...
export const LegalDocumentsPanel = () => {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError('');
      try {
        const data = user?.role === 'worker' ? await workerApi.listDocuments() : await clientApi.listDocuments();
        if (mounted) setDocs(Array.isArray(data) ? data : []);
      } catch (err) {
        if (mounted) setError(err.message || 'Could not load documents.');
      } finally {
  ...
  return (
    <div className="card invoice-list">
      ...
      <p className="muted">
        Official contract and invoice records generated by the system and delivered to both parties.
      </p>
      {loading && <p className="muted">Loading documents…</p>}
      {error && <p className="error-text">{error}</p>}
      {!loading && !error && docs.length === 0 && <p className="muted">No documents yet.</p>}
      {!loading && !error && docs.length > 0 && (
        <ul>
          {docs.map((doc) => (
            <li key={doc.id} className="invoice-item">
              <div>
                <h4>{doc.title}</h4>
                <p className="muted">
                  {doc.documentType} • job {doc.jobId} • {new Date(doc.createdAt).toLocaleString()}
                </p>
              </div>
              <div className="job-buttons">
                <button type="button" className="btn small primary" onClick={() => openDoc(doc)}>
                  Open
                </button>
                <button type="button" className="btn small secondary" onClick={() => downloadDoc(doc)}>
                  Download
                </button>
              </div>
            </li>
